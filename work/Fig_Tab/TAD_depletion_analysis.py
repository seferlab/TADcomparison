...
def enrichment_analysis(chrname,chrsize,resolution_kb,TADfile,peak_file):
    ...
    for pi in range(0, Peaks_chr.shape[0]):
        ...
        boundary_peak = extended_boundary.loc[
            (extended_boundary['right'] >= left_peak) & (extended_boundary['left'] <= right_peak)]
...
def extract(element,peaks_bedfile):
    ...
    for caller in callers:
        ...
        for data in dataset:
            ...
            valley_minimum = np.max(mean_peak[int(base_interval / 2 - base_interval * 0.01):int(base_interval / 2 + base_interval * 0.01)])
            # print(peak_maximum)
            ...
            # print(fc)
            pval_peak=st.norm.cdf(peak_val, loc=bg_val, scale=np.std(bg))
            # print(pval_peak)
            ...
            res_caller[:,dataset.index(data)]=[valley_minimum,boundary_tagged_ratio,fc,ji_element,ji_border,pval_peak]
            # plt.close('all')
            # fig, ax1 = plt.subplots()
            # ax1.plot(range(0, 100), mean_peak, linewidth=1, color='darkslategray',marker='o', markersize=0)
            # ax1.set_xticks(list(range(0, 101, 25)))
            # ax1.set_xticklabels(['-500kb', '-250kb', '0', '+250kb', '+500kb'])
            # ax1.set_title(element)
            # ax1.set_ylabel('Average number of peaks per 10kb')
            # plt.tight_layout()
            # plt.savefig('./out/'+element+'_'+caller+'_'+dataset+'.pdf')
    ...
    for i in range(0,5):
        ...
        np.savetxt('./output/' + element + '_' + comparing_items[i] + '.txt', data_to_plot)  # 缺省按照'%.18e'格式保存数据，以空格分隔
    ...
    np.savetxt('./output/' + element + '_' + comparing_items[5] + '.txt', data_to_plot)  # 缺省按照'%.18e'格式保存数据，以空格分隔


extract('H3K9me3','marks/wgEncodeBroadHistoneGm12878H3k9me3StdPk.broadPeak')
