...
def enrichment_analysis(chrname,chrsize,resolution_kb,TADfile,peak_file):
    ...
    covered_peak = 0
    for pi in range(0, len(Peaks_chr)):
        TSS_peak = Peaks_chr[pi]
        boundary_peak = extended_boundary.loc[(extended_boundary['right'] >= TSS_peak) & (extended_boundary['left'] <= TSS_peak)]
        if boundary_peak.shape[0] > 0:
            covered_peak += 1
    element_covered_ratio = covered_peak / len(Peaks_chr)
    ...
    if Enrich_all is None:
        ...
        #ji_element = covered_peak / (len(Peaks_chr) + Enrich_all.shape[1])
        #ji_border = sum(ratio_tmp2) / (len(Peaks_chr) + Enrich_all.shape[1])
        # print(boundary_tagged_ratio)

        enrich_line=Enrich_all.mean(axis=1)
        return covered_peak, sum(ratio_tmp2), (len(Peaks_chr) + Enrich_all.shape[1]), Enrich_all.shape[1], enrich_line
...
def extract(element,peaks_bedfile):
    ...
    for caller in callers:
        ...
        for data in dataset:
            ...
            boundary_assemble = []
            for chr in chrs:
                if os.path.exists('../all_TADs/loci/%s/%s_%s.chr%s'%(caller,data,caller,chr))==True:
    ...
    #     tick.set_rotation(90)
    # plt.tight_layout()
    # plt.savefig('./out/' + element + ' ' + comparing_items[3] + '.pdf')
    np.savetxt('output/%s_%s.txt'%(element,comparing_items[5]), data_to_plot)


extract('TSS','marks/TSS_all.txt')
